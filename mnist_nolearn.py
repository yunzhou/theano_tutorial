...
def main_nolearn(model='mlp', num_epochs=50):
    ...
    layers=[  # three layers: one hidden layer
        ...
        ('inputDrop', layers.DropoutLayer),
        ('hidden1', layers.DenseLayer),
        ('hidden1Drop', layers.DropoutLayer),
        ('hidden2', layers.DenseLayer),
        ('hidden2Drop', layers.DropoutLayer),
        ('output', layers.DenseLayer),
        ],
    # layer parameters:
    input_shape=(None, 1, 28, 28),  # 28x28 input pixels per batch

    inputDrop_p=0.2, # Apply 20% dropout to the input data:

    # Add a fully-connected layer of 800 units, using the linear rectifier, and
    # initializing weights with Glorot's scheme (which is the default anyway):
    hidden1_num_units=800,  # number of units in hidden layer
    hidden1_nonlinearity=lasagne.nonlinearities.rectify,
    hidden1_W=lasagne.init.GlorotUniform(),

    hidden1Drop_p=0.5,# We'll now add dropout of 50%:
    ...
    hidden2Drop_p=0.5,# We'll now add dropout of 50%:
    ...
    max_epochs=num_epochs,  # we want to train this many epochs
    verbose=1,
    train_split=DummySplit(X_train, y_train, X_val, y_val)
    )
